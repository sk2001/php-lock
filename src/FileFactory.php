...
class FileFactory implements Factory
{
     ...
     * @param bool   $blocking  true to wait for lock to be available, false to throw exception instead of waiting
     *
     * @return FileLock
     */
    ...
    )
    {
        if (!is_dir($this->lock_dir)
            && !mkdir($concurrentDirectory = $this->lock_dir, 0777, true)
